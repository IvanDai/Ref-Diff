...
from refdiff import ConditionalUNet1D, GaussianDiffusion
from refdiff.data import iq_to_spectrum
...
def merge(a,b):
    ...
    return out
def inv(s):
    z=torch.complex(s[0],s[1]); x=torch.fft.ifft(torch.fft.ifftshift(z),norm="ortho"); return torch.stack((x.real,x.imag)).float()
def output_dir():
...
def main():
    ...
    dev=cfg.get("device","auto"); device=torch.device(("cuda" if torch.cuda.is_available() else "cpu") if dev=="auto" else dev); weights=Path(a.weights or cfg["weights"]); condition=iq_to_spectrum(y).unsqueeze(0).to(device); target=iq_to_spectrum(x); model,diff=load_model(weights,cfg,device); steps=min(int(cfg.get("ddim_steps",50)),diff.timesteps)
    ...
    for row,(name,v) in enumerate((("y_rx",inv(raw)),("x_ref",inv(target)),("RefDiff output",inv(restored)))): ax[row,0].plot(v[0].numpy(),lw=.8); ax[row,0].set_ylabel(name); ax[row,0].set_title("real"); ax[row,1].plot(v[1].numpy(),lw=.8); ax[row,1].set_title("imag")
    ...
    (out/"metadata.json").write_text(json.dumps({"weights":str(weights.resolve()),"modulation":mod,"esn0_db":snr,"seed":seed,"ddim_steps":steps,**meta},indent=2)); print(f"source={meta['source']}\noutput={out.resolve()}")
